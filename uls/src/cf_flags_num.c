...
static inline void fill_bool(char *flags_char, bool *flags, int count_flags) {
    ...
    for (int i = 0; i < ALL_FLAGS; i++) // булевый массив - обнулить
        flags[i] = false;
    for (int i = count_flags - 1 ; i >= 0; i--) // снести дубликаты в строке flags
        for (int j = i - 1; j >= 0; j--)
            if (flags_char[i] == flags_char[j] && flags_char[i] != '-')
                flags_char[j] = '-';
    for (int i = 0, j = 0; i < count_flags; i++) // дать правдивое значение флага в булевом массиве
